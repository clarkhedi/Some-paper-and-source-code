...
train_labels    = labels(trainset);  % contains the labels of the trainset 240*1 double
train_data      = BOW(:,trainset)';  % contains the train data 240*300
...
%第一个分类是：'-s 0 -t 4 -c %f -b 1 -g %f -q'
%-s 0：svm类型是C-SVC；  -t 2:RBF核；   -c %f：惩罚因子；   
%-b 1：概率估计，是否计算SVC的概率估计；
%-g %f：设置核函数中gamma的值   -q？？？？？？
