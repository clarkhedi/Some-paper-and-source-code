...
%% replicate border image pixels to the outer area 将边框图像像素复制到外部区域
I0(1:R,:,:) = repmat(I0(R+1,:,:), [R,1,1]); %%（按行）复制第四行到前三行
I0(end-R+1:end,:,:) = repmat(I0(end-R,:,:), [R,1,1]); %%（按行）复制倒数第四行到后三行
I0(:,1:R,:) = repmat(I0(:,R+1,:), [1,R,1]); %%（按列）复制第四列到前三列
I0(:,end-R+1:end,:) = repmat(I0(:,end-R,:), [1,R,1]); %%（按列）复制倒数第四列到后三列
