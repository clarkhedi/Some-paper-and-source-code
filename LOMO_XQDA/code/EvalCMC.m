...
binaryLabels = bsxfun(@eq, galLabels, probLabels); %对角线上元素为1 match / non-match labels corresponding to the score matrix
...
clear binaryLabels
[~, maxIndex] = max(score); % ~为包含每一列的最大值的行向量 get the location of the maximum genuine score
