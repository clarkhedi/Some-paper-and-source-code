...
for k = 1 : c
    ...
    nk = length(galIndex);
    galClassSum(k, :) = sum( galX(galIndex, :), 1 );%%对应论文中的类内协方差矩阵中的S
    
    probIndex = find(probLabels == labels(k));
    mk = length(probIndex);
    probClassSum(k, :) = sum( probX(probIndex, :), 1 );%%对应论文中的类内协方差矩阵中的R
...
t0 = tic;
[V, S] = svd(inCov \ exCov); %%对应论文中J(w)的最大化的解:inv(incov)*excov
% V为特征向量，S为特征值
if verbose == true
    fprintf(' %.3g seconds.\n', toc(t0));
end

latent = diag(S); %%统计inv(incov)*excov的特征值
...
V = V(:, index(1:qdaDims)); %%对应论文中的子空间W
if ~exist('W', 'var');
    W = V;
else
    W = W * V;
...
inCov = V' * inCov * V;
exCov = V' * exCov * V;
M = inv(inCov) - inv(exCov);%%得到对应论文中学习的核矩阵M(W)
